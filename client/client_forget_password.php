...
if (isset($_POST['reset_password'])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $checkEmail = $mysqli->prepare("SELECT * FROM `iB_clients` WHERE `email` = ?");
        ...
        if ($result->num_rows > 0) {
            ...
            $query = "UPDATE iB_clients SET otp=?, otp_expiry=DATE_ADD(NOW(), INTERVAL 10 MINUTE) WHERE email=?";
...
<body class="hold-transition login-page">
    <div class="login-box">
        <div class="login-logo">
            <p>DigiBankX - Client Password Reset</p>
        </div>
        <div class="card">
            <div class="card-body login-card-body">
                ...
                <p class="mt-2">
                    <a href="PAGES_CLEI.php">Back to Login</a>
                </p>
            </div>
        </div>
    </div>
